...
void main(void)
{
    ...
    PM5CTL0 &= ~LOCKLPM5; //Needed for FR devices
...
void startTimer(int freq) //function to set a particular frequency
{
    ...
    TB0CCTL0 = CCIE; //Enable interupt for Capture Compare
    TB0CTL = TBSSEL_2 + MC_1 + ID_3;  //Choose SMCLK (1000000Hz) divided by 8 = 125000, in Upcount Mode

    t = (125000 / freq); // Divides the clock by the desired frequency to give t --> 125000 / 10 = 12500
    TB0CCR0 = t;         // Set capture comp reg 0 to 12500. --> 125000/12500 gives desired 10Hz
}
...
{
    if (eButton != 0x00) {
     if (P1IN & BUTTON) {  // Triggers interrupt when Button pressed
        touchButton = 0;
        eButton = 0;
        count = 0;
    }
